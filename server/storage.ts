...
export class MemStorage implements IStorage {
  ...
  private seedData() {
    ...
    // Create donor family relationships
    this.donorFamilies.set("family-1", {
      id: "family-1",
    ...
    this.donorFamilies.set("family-2", {
      id: "family-2", 
      patientId: "patient-profile-1",
    ...
    // Create sample transfusions
    this.transfusions.set("transfusion-1", {
      id: "transfusion-1",
      patientId: "patient-profile-1",
      donorId: "donor-profile-1",
    ...
    this.notifications.set("notification-1", {
      id: "notification-1",
      userId: "patient-1",
      title: "Upcoming Transfusion",
      message: "Your next transfusion is scheduled for March 15th",
    ...
    this.emergencyRequests.set("emergency-1", {
      id: "emergency-1",
      patientId: "patient-profile-1",
      urgencyLevel: "high",
      status: "active",
  ...
  async getDonorsByBloodGroup(bloodGroup: string): Promise<Donor[]> {
    const users = Array.from(this.users.values()).filter(u => u.bloodGroup === bloodGroup);
    const donors = Array.from(this.donors.values()).filter(d => 
      users.some(u => u.id === d.userId)
    );
    return donors;
  }

  async getAvailableDonors(bloodGroup: string, location?: string): Promise<Donor[]> {
    return this.getDonorsByBloodGroup(bloodGroup);
  }
  ...
  async getUpcomingTransfusions(): Promise<Transfusion[]> {
    return Array.from(this.transfusions.values()).filter(t => t.status === 'scheduled');
  }
